...
const github = env.AUTH_GITHUB_ID !== undefined && env.AUTH_GITHUB_SECRET !== undefined
 ...
 * @returns The response.
 */
export async function validateGithubCallback(request) {
    if (!github) {
        return new Response(null, {
            status: 404,
            statusText: 'Not Found',
    ...
    try {
        ...
        if (!githubUser.email) {
            return new Response('Email not found. Please set public email in GitHub.', {
                status: 400,
            });
        }
        const existingUser = await db.query.oauthAccountTable.findFirst({
            where: (table, { and, eq }) => and(eq(table.providerId, 'github'), eq(table.providerUserId, githubUser.id)),
        });
        if (existingUser) {
