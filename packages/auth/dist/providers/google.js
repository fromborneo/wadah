...
const google = env.AUTH_GOOGLE_ID !== undefined &&
    env.AUTH_GOOGLE_SECRET !== undefined &&
    new Google(env.AUTH_GOOGLE_ID, env.AUTH_GOOGLE_SECRET, `${baseUrl}/login/google/callback`);
/**
 ...
 * @returns The response.
 */
export async function validateGoogleCallback(request) {
    if (!google) {
        return new Response(null, {
            status: 404,
            statusText: 'Not Found',
    ...
    try {
        ...
        const existingUser = await db.query.oauthAccountTable.findFirst({
            where: (table, { and, eq }) => and(eq(table.providerId, 'google'), eq(table.providerUserId, sub)),
        });
        if (existingUser) {
